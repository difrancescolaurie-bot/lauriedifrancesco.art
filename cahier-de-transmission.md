...
**N.4 — Toward the Blue, structure ouverte en attente.** Audit des 11 images de la série : toutes ont un ratio 1.5004 (paysage 3:2, 2000 × 1333 px). Aucune image n'est verticale ni carrée. Les seuils calculés sont : ratio photo < 0.94 pour que le Rilke intégral tienne en regard d'une photo ; ratio photo < 1.37 pour le Nietzsche intégral. La série ne contient aucune image satisfaisant ces seuils. La structure d'ouverture TTB est une question éditoriale en attente de validation : l'ancienne version (Rilke + Nietzsche courts, côte à côte avec les photos 1 et 2) est intacte dans les fichiers EN et lang. Ne pas modifier TTB avant décision explicite.
...
**O.1 — Nouveau modèle de rangée (décision, implémentation en attente de maquette).** Le modèle actuel (ratio de cellule texte → hauteur de rangée variable → image qui ne remplit pas sa cellule) est remplacé. Nouveau principe : une constante de page `H` (hauteur de rangée) ; la photo occupe exactement `H` px de hauteur, sa largeur vaut `ratio × H`, l'image remplit sa cellule sans vide résiduel. La cellule texte prend toute la largeur restante (`791 − ratio × H − 16 px` de gouttière). Le système `data-ratio 0.75 / 1 / 1.3333` disparaît. Si le texte est plus haut que `H`, la rangée grandit et la photo se centre verticalement en face du texte — ce n'est pas un défaut, c'est le comportement attendu. Calculs fournis pour Rilke (~600 chars, cit DROITE) et Nietzsche (~270 chars, 2 §, cit GAUCHE) à H = 300, 340, 380, 420 px : Rilke déborde à tous les ratios (rangée grandit de +167 à +749 px selon H ; H=300 donne le dépassement minimal) ; Nietzsche tient à H=300 (+33 px), H=340 (+19 px), H=380 (5 px — limite) et déborde à H=420. H=340 est le meilleur compromis. Implémentation suspendue : maquette HTML de référence attendue avant tout code.
